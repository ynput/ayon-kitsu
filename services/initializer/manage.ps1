...
$BASE_NAME = "kitsu-initializer"
$ADDON_VERSION = "1.0"
$IMAGE_FULL_NAME = "$($BASE_NAME):$($ADDON_VERSION)"
$BASH_CONTAINER_NAME = "$($BASE_NAME)-bash-$($ADDON_VERSION)"

function defaultfunc {
  Write-Host ""
  Write-Host "*************************"
  Write-Host "AYON kitsu initializer service"
  Write-Host "   Run initializer service"
...
function run {
    load-env
    & poetry run python -m initializer
}
...
function dev {
  ...
  & docker run --rm -ti `
    -v "$($script_dir):/service" `
  	...
  	"$($IMAGE_FULL_NAME)" python -m initializer
}
